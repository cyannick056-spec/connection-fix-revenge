...
class GalleryRepository(context: Context) {
    ...
    fun save(items: List<ModItem>) { cache = AppCache(items, System.currentTimeMillis()); store.write("gallery.json", cache) }
    fun updateOne(item: ModItem) { save(cache.items.map { if (it.id == item.id) item else it }) }
    ...
    suspend fun resolveHd(item: ModItem): ModItem = withContext(Dispatchers.IO) {
        ...
        ) + doc.select("img").mapNotNull { el -> (el.attr("abs:src").ifBlank { el.attr("abs:data-src") }).takeIf { it.contains("postimg") } }
        val hd = candidates.firstOrNull { it.contains("i.postimg.cc") } ?: candidates.firstOrNull().orEmpty()
    ...
    private fun resolveHdNoSave(item: ModItem): ModItem {
        ...
        doc.selectFirst("meta[property=og:image]")?.attr("abs:content")?.takeIf { it.isNotBlank() }?.let(candidates::add)
        doc.selectFirst("link[rel=image_src]")?.attr("abs:href")?.takeIf { it.isNotBlank() }?.let(candidates::add)
        doc.select("img").forEach { el -> val u = el.attr("abs:src").ifBlank { el.attr("abs:data-src") }; if (u.contains("postimg")) candidates.add(u) }
        val hd = candidates.firstOrNull { it.contains("i.postimg.cc") } ?: candidates.firstOrNull().orEmpty()
    ...
    fun attachMega(matches: Map<String, MegaFile>): List<ModItem> {
        val updated = cache.items.map { item ->
            ...
            if (f == null) item.copy(megaName="",megaHandle="",megaKeyBase64="",megaIvBase64="",megaSize=0L,megaScore=0.0)
            else item.copy(megaName=f.name,megaHandle=f.handle,megaKeyBase64=f.fileKeyBase64,megaIvBase64=f.ivBase64,megaSize=f.size)
        }
        save(updated); return updated
    }

    companion object { private const val UA = "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 Chrome/140 Mobile Safari/537.36" }
}
