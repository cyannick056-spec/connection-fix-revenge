...
class MegaPublicFolderClient {
    private val http = OkHttpClient.Builder().connectTimeout(25, TimeUnit.SECONDS).readTimeout(60, TimeUnit.SECONDS).build()
    data class FolderRef(val handle: String, val key: ByteArray)

    fun parseFolderUrl(url: String): FolderRef {
        val m = Regex("/folder/([A-Za-z0-9_-]+)#([A-Za-z0-9_-]+)").find(url) ?: error("Enlace de carpeta MEGA no válido")
    ...
    suspend fun listFiles(url: String, onProgress: (Int, String) -> Unit = { _, _ -> }): List<MegaFile> = withContext(Dispatchers.IO) {
        ...
        onProgress(10, "Leyendo carpeta pública de MEGA…")
        val result = api(ref.handle, JSONObject().apply { put("a", "f"); put("c", 1); put("r", 1); put("ca", 1) })
        ...
        for (i in 0 until nodes.length()) {
            ...
            for (keyMaterial in decryptNodeKeyCandidates(rawKey, ref.key)) {
                val actualKey = if (type == 0 && keyMaterial.size >= 32) ByteArray(16) { idx -> (keyMaterial[idx].toInt() xor keyMaterial[idx + 16].toInt()).toByte() } else keyMaterial.copyOfRange(0, 16)
                val iv = if (type == 0 && keyMaterial.size >= 24) ByteArray(16).also { dst -> keyMaterial.copyInto(dst, 0, 16, 24) } else ByteArray(16)
                val candidateName = decryptAttributes(attrs, actualKey).orEmpty()
                if (candidateName.isNotBlank()) { name = candidateName; chosenKey = actualKey; chosenIv = iv; break }
            }
            ...
            if (i % 100 == 0) {
                val pct = 15 + (45.0 * i / maxOf(1, nodes.length())).toInt()
                onProgress(pct, "Descifrando MEGA… $i/${nodes.length()}")
            }
        }
        fun buildPath(node: DecodedNode): String {
            val parts = mutableListOf<String>(); var cur: DecodedNode? = node; val guard = mutableSetOf<String>()
            while (cur != null && guard.add(cur.handle)) { parts += cur.name; cur = decoded[cur.parent] }
            return parts.asReversed().joinToString("/")
        }
        val files = decoded.values.asSequence().filter { it.type == 0 && it.name.matches(Regex("(?i).+\\.(7z|zip|rar|tmc|tmcl|zipmod|pak)$")) }.map { n ->
            val path = buildPath(n)
            MegaFile(n.handle, n.parent, n.name, path, TextUtils.detectCharacter(path), n.size, Base64.encodeToString(n.key, Base64.NO_WRAP), Base64.encodeToString(n.iv, Base64.NO_WRAP))
        }.toList()
        onProgress(100, "MEGA: ${files.size} archivos encontrados")
        files
    }
    ...
    suspend fun getDownloadInfo(folderUrl: String, nodeHandle: String): MegaDownloadInfo = withContext(Dispatchers.IO) {
        ...
        var obj = api(ref.handle, JSONObject().apply { put("a", "g"); put("g", 1); put("n", nodeHandle) })
        var direct = obj.optString("g")
        if (direct.isBlank()) {
            obj = api(ref.handle, JSONObject().apply { put("a", "g"); put("g", 1); put("p", nodeHandle) })
    ...
    suspend fun downloadTo(folderUrl: String, nodeHandle: String, fileKey: ByteArray, iv: ByteArray, output: java.io.OutputStream, onProgress: (Int, Long, Long) -> Unit) = withContext(Dispatchers.IO) {
        ...
        http.newCall(req).execute().use { response ->
            ...
            cipher.init(Cipher.DECRYPT_MODE, SecretKeySpec(fileKey.copyOfRange(0, 16), "AES"), IvParameterSpec(iv.copyOf(16)))
            body.byteStream().use { input -> output.use { out ->
                val buffer = ByteArray(128 * 1024); var read: Int; var done = 0L; var lastPct = -1
                ...
                val tail = cipher.doFinal(); if (tail.isNotEmpty()) out.write(tail); out.flush(); onProgress(100, done, total)
            }}
        }
    }
    ...
    private fun api(folderHandle: String, command: JSONObject): JSONObject {
        ...
        val text = http.newCall(req).execute().use { response -> if (!response.isSuccessful) error("MEGA HTTP ${response.code}"); response.body?.string().orEmpty() }
        val arr = JSONArray(text); val first = arr.opt(0)
    ...
    private fun decryptNodeKeyCandidates(raw: String, folderKey: ByteArray): List<ByteArray> {
        ...
        return raw.split('/').mapNotNull { part ->
            ...
            runCatching { aesEcbDecrypt(folderKey.copyOfRange(0, 16), enc) }.getOrNull()?.takeIf { it.size == 16 || it.size == 32 }
        }
    }
    ...
    private fun decryptAttributes(raw: String, key: ByteArray): String? {
        ...
        return runCatching {
            ...
            cipher.init(Cipher.DECRYPT_MODE, SecretKeySpec(key.copyOfRange(0, 16), "AES"), IvParameterSpec(ByteArray(16)))
    ...
    private fun aesEcbDecrypt(key: ByteArray, data: ByteArray): ByteArray {
        val cipher = Cipher.getInstance("AES/ECB/NoPadding"); cipher.init(Cipher.DECRYPT_MODE, SecretKeySpec(key, "AES")); return cipher.doFinal(data)
    }
    private fun b64UrlDecode(input: String): ByteArray {
        var s = input.replace('-', '+').replace('_', '/'); while (s.length % 4 != 0) s += "="; return Base64.decode(s, Base64.DEFAULT)
    }
    private data class DecodedNode(val handle: String, val parent: String, val type: Int, val name: String, val size: Long, val key: ByteArray, val iv: ByteArray)
